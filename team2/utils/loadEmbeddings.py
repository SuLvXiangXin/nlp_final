...
class LoadEmbeddings():
    ...
    def loadEmbeddings(self, filepath, data_path, vocab_size, binary_val):
        ...
        if not os.path.exists(embed_short):
            ...
            fp = np.memmap(embed_short, dtype=np.double, mode='w+', shape=wv.vectors.shape)
            fp[:] = wv.vectors[:]
            with open(os.path.normpath("%s/embed.vocab" % data_path), "w", encoding='utf-8') as fp:
                for _, w in sorted((voc.index, word) for word, voc in wv.key_to_index.items()):
                    fp.write('%s\n' % w)
            del fp, wv

        self.W = np.memmap(os.path.normpath("%s/embed.dat" % data_path), dtype=np.double, mode="r",
