...
class TfidfFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        xHeadlineTfidf = vecH.fit_transform(df['Headline_unigram'].map(lambda x: ' '.join(
            x)))  # use ' '.join(Headline_unigram) instead of Headline since the former is already stemmed
        ...
        cos = cosine_sim(xHeadlineTfidf, xBodyTfidf)
        print(cos)
        print(cos.shape)
        simTfidf = np.asarray(cos)[:, np.newaxis]
    ...
    def read(self, header='train', train=True):
