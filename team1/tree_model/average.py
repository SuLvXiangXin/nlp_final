...
def load_data():
    yuxi = pd.read_csv('predtest_cor2.csv',
                       usecols=['Headline', 'Body ID', 'Stance', 'prob_0', 'prob_1', 'prob_2', 'prob_3'])
    print('yuxi.shape:')
    print(yuxi.shape)
...
def loadTest():
    yuxi = pd.read_csv('tree_pred_prob_cor2.csv',
                       usecols=['Headline', 'Body ID', 'prob_0', 'prob_1', 'prob_2', 'prob_3'])
    print('yuxi.shape:')
    print(yuxi.shape)

    # doug = pd.read_csv('dosiblOutputFinal.csv',
    #                    usecols=['Headline', 'Body ID', 'Agree', 'Disagree', 'Discuss', 'Unrelated'])
    doug = pd.read_csv('../deep_learning_model/deepoutput.csv',encoding='ISO-8859-1',
                       usecols=['Headline', 'BodyID', 'Agree', 'Disagree', 'Discuss', 'Unrelated'])
...
def stack_test():
    ...
    pred_y = np.hstack([pred_agree.reshape((-1, 1)), pred_disagree.reshape((-1, 1)), pred_discuss.reshape((-1, 1)),
                        pred_unrelated.reshape((-1, 1))])
    ...
    stances = pd.read_csv("competition_test_stances_unlabeled_processed.csv")
