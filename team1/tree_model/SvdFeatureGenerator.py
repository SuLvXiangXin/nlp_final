...
class SvdFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        featuresTrain = tfidfGenerator.read('train', train=False)
        ...
        if n_test > 0:
            ...
            featuresTest = tfidfGenerator.read('test', train=False)
        ...
        cos = cosine_sim(xHeadlineSvd, xBodySvd)
        simSvd = np.asarray(cos)[:, np.newaxis]
        print('simSvd.shape:')
        print(simSvd.shape)
