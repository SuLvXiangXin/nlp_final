...
class Word2VecFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        model = gensim.models.KeyedVectors.load_word2vec_format('../deep_learning_model/GoogleNews-vectors-negative300.bin', binary=True)
        print ('model loaded')
        ...
        simVec = np.asarray(list(map(cosine_sim, headlineVec, bodyVec)))
        print ('simVec.shape:')
        print (simVec.shape)
