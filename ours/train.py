...
train_set, val_set = torch.utils.data.random_split(train_dataset,
                                                   [len(train_dataset) - int(len(train_dataset) * val_size),
...
for epoch in range(start_epoch, epochs):
    # train step
    # record the average/sum of metrics
    loss_sum = 0
    acc_sum = 0
    score_sum = 0
    score_all = 0
    ...
    # save the checkpoint
    torch.save(
        {'model': model.state_dict(), 'optim': optimizer.state_dict(), 'train': [losses, accs], 'epoch': epoch + 1,
    ...
    if score_sum_val > best_score:
        ...
        best_epoch = epoch + 1
        torch.save(
            {'model': model.state_dict(), 'optim': optimizer.state_dict(), 'train': [losses, accs], 'epoch': epoch,
             'best_score': best_score},
